...
After the reset, Bluetooth profile 0 (the first Bluetooth profile) is the
default Linux profile. The Mac profile is secondary: hold the lower and raise
thumb keys together, then tap `2` on the top row to select it. To return to
Linux, use the same layer combination and tap `1`.
